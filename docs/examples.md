...
```typescript
// kit.config.ts
import { kitConfig, defineContentType, defineTaxonomy } from '@webicient/sanity-kit';
...
```typescript
// kit.config.ts
import { kitConfig, defineContentType, defineTaxonomy, defineModule } from '@webicient/sanity-kit';
...
export async function generateSEOMetadata({
  ...
  const imageUrl = metaImage ? urlForImage(metaImage).width(1200).height(630).url() : undefined;
...
export const getCachedPage = cache(async (slug: string, locale: string = 'en') => {
  return loadQuery(
